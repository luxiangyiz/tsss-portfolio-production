/**
 * 3D CircularGallery — 基于 OGL WebGL 的圆弧旋转画廊组件
 *
 * 源码来源：@react-bits/CircularGallery-JS-CSS（用户提供的动效代码）
 * 适配：整合到 WordPress 项目归档页，支持从 REST API 动态加载项目数据
 */
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    this.medias = this.mediasImages.map((data, index) =>
      new Media({
        ...
        borderRadius,
        font,
        onClick: data.onClick || this.onItemClick,
      }),
    );
  }
...
/*  React 导出组件                                                     */
/* ------------------------------------------------------------------ */
const { createElement: h, useRef, useEffect, useState } = window.wp.element;
...
export default function CircularGallery3D({
  ...
  const className = 'circular-gallery-3d' + (ready ? ' is-ready' : '');
  return h('div', {
    className: className,
    ref: containerRef,
    tabIndex: 0,
    role: 'region',
    'aria-label': '项目画廊。使用左右方向键或拖拽浏览。',
